...
function [xstar, its] = IAPG_shifted(x_sdp, fs, grad_fs, constraints, varargin)
    ...
    if isempty(options.max_delay)
        b = ceil(m/10);
    else
        assert(isnumeric(options.max_delay), 'Batchsize must be number');
        b = options.max_delay;
    end
    ...
    if verbose
        p = progress('Compiling solvers', 2*m);
    end
    ...
    past_gradients = zeros(d(1), d(2), b);
    past_subgradients = zeros(d(1), d(2), b);
    ...
    for k = 1:m
        ...
        zk = xk - ak * (grad_fs{i}(xk) + past_gradients + past_subgrads);
