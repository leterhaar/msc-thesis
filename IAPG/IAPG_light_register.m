...
%   - max_delay     : maximum delay (default 5)


function [xstar, its] = IAPG_light_register(x_sdp, fs, grad_fs, constraints, varargin)
    ...
    if verbose
        p = progress('Preparing IAPG-l-r', m);
    end
    ...
    gradients = zeros(d(1), d(2), m);
    
    % calculate batch size for updating gradient info
    batch_size = ceil(m/options.max_delay);

    %% start main iterations

    if verbose
        p = progress('Iterating IAPG-l-r', options.max_its);
    end
    ...
    for k = 1:options.max_its
        ...
        % calculate which agents should update their IG 
        from = rem(batch_size * k, m)+1;
        to = min(from + batch_size - 1, m);
        
        % 'recieve' updated gradients from max_delay ago
        delay = min(k-1, options.max_delay);
        for i = from:to
            gradients(:,:,i) = grad_fs{i}(its(k-delay).x);
        end
           
        % sum up all gradients and calculate zk
        zk = xk - ak * sum(gradients, 3);
        
        % select a random agent for projection
        i = randi(m);
