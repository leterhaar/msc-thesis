...
%   - batch_size    : integer for batch size (default m/10)


function [xstar, its] = IAPG_batch(x_sdp, fs, grad_fs, constraints, varargin)
    ...
    if isempty(options.batch_size)
        B = ceil(m/10);
    else
        assert(isnumeric(options.batch_size), 'Batchsize must be number');
    ...
    if verbose
        p = progress('Compiling solvers', m);
    end
    ...
    its(1).x = x0;
    past_gradients = nan(d(1), d(2), m);
    
    % loop over all agents and store
    for i = 1:m
        past_gradients(:,:,i) = grad_fs{i}(x0);
    end

    %% start main iterations

    if verbose
        p = progress('Iterating IAPG batch', options.max_its);
    end
    ...
    for k = 1:options.max_its
        ...
        xk = its(k).x;
        ak = options.alpha(k);
        batch_constraints = [options.default_constraint];
        
        % select a random batch of agents
        current_batch = randperm(m, B);
        
        % update the subgradients for the selected batch and build
        % constraints
        for i = current_batch
            past_gradients(:,:,i) = grad_fs{i}(xk);
                        
            li_k(i) = k;
        end
        
        zk = xk - ak * sum(past_gradients,3);
        
        % merge solvers and solve proximal
        merged_solver = [proximals{current_batch}];
        [next_x, problem, msg] = merged_solver(ak, zk);
