...
% create feasible solution (check if this is already done by previous test)

if not(exist('xstar', 'var') == 1)
...
flag = AC_check({xstar{1}, eye(n), xstar{3:4}}, ac, wind, t);
assert(flag == 3, 'Ws is not rank 1');
flag = AC_check({xstar{1:2}, eye(n), xstar{4}}, ac, wind, t);
assert(flag == 3, 'Ws is not rank 1');
...
assert(flag == 4, 'Should be infeasible');
